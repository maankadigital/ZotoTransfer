...
export const Header: React.FC = () => {
  ...
  return (
    <header className="bg-white/90 dark:bg-[#1a1a1f]/90 backdrop-blur-md border-b border-slate-200/60 dark:border-white/[0.06] shadow-[0_2px_15px_rgba(0,0,0,0.03)] dark:shadow-[0_2px_20px_rgba(0,0,0,0.3)] sticky top-0 z-50">
      <div className="flex justify-between items-center w-full px-lg py-sm max-w-container-max mx-auto">

        {/* Brand */}
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setView('home')}>
          <Logo className="h-9 shrink-0" variant="full" isWhite={dark} />
        </div>

        {/* Nav links (home & send views) */}
        {(currentView === 'home' || currentView === 'send') && (
          <nav className="hidden md:flex items-center gap-0.5 bg-slate-100/70 dark:bg-white/[0.05] p-1 rounded-full border border-slate-200/60 dark:border-white/[0.08]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-full border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.06] hover:bg-slate-100 dark:hover:bg-white/10 text-slate-600 dark:text-white/80 transition-all hover:scale-105 active:scale-95"
          >
          ...
          {currentUser ? (
            <div className="flex items-center gap-3">
              ...
              <img
                alt="User profile"
                className="w-10 h-10 rounded-full object-cover border border-slate-200 dark:border-white/10 cursor-pointer shrink-0"
                onClick={() => setView((currentUser.role === 'super_admin' || currentUser.role === 'admin') ? 'admin' : 'dashboard')}
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="text-slate-700 dark:text-white/85 hover:text-primary font-semibold text-xs py-2 px-3 transition-colors"
              >
                Sign In
              </button>
              <button
